...
alt_u32 calc_conv_enc_sw(alt_u32 reg, int *ticks)
{
    ...
    t1 = alt_timestamp();
    overhead = t1 - t0;
    //printf("Overhead of alt_timestamp(): %d   \n", overhead);
...
int main(void){
    ...
    int samples = 10000;
    int i;
    IOWR(CONV_ENC_BASE, CONV_ENC_FW_RST, 0); // write dummy data to reset the accelerator
    data = 0x00000041; // initial data
    //data = 0xFFFFFFFF; // initial data
    ...
    // sanity check
    if(val_sw != val_hw)
        printf("Error: data value: %X, SW result: %X, HW result: %X \n", data, val_sw, val_hw);
    ...
    */
    ...
    for (i = 0; i < samples; i++){
    	// SW & HW reset
    	// calc_sw_state = '0';
    	// IOWR(CONV_ENC_BASE, CONV_ENC_FW_RST, 0);
    	// start calc
        val_sw = calc_conv_enc_sw(data, &ticks);
        val_hw = calc_conv_enc_hw(CONV_ENC_BASE, data, &clks);
        // sanity check
        if(val_sw != val_hw)
            printf("Error: data value: 0x%08X, SW result: 0x%08X, HW result: 0x%08X ; Round #: %d \n", data, val_sw, val_hw, i);
        // store execution cycles values
        s = s + ticks;
        c = c + clks;
