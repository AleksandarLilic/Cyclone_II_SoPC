...
* note:
***********************************************************************/
void flashsys_init_v2(alt_u32 btn_base, alt_u32 ledr_base, status_struct *status)
{
...
void timer_inc(status_struct *status)
{
    ...
    if ((now - last) < 1000) // interval not reached (1000ms period)
        return;
    last = now;
    timer_fired = 1;
...
int main(){
    ...
    // initial settings
    flashsys_init_v2(BTN_BASE, LEDR_BASE, &status);
